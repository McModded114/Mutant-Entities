...
public class MutantEndermanEntity extends EnderMan implements IAnimatable {
	...
	public MutantEndermanEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(MutantEntitiesModEntities.MUTANT_ENDERMAN.get(), world);
	}

	public MutantEndermanEntity(EntityType<MutantEndermanEntity> type, Level world) {
		super(type, world);
		xpReward = 80;
		setNoAi(false);
	}
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "mutant_enderman");
	}
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty, MobSpawnType reason, @Nullable SpawnGroupData livingdata, @Nullable CompoundTag tag) {
		...
		EndermanProcedure.execute(this);
		return retval;
	}
	...
	public static void init() {
		SpawnPlacements.register(MutantEntitiesModEntities.MUTANT_ENDERMAN.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
	...
	private <E extends IAnimatable> PlayState movementPredicate(AnimationEvent<E> event) {
		if (this.animationprocedure.equals("empty")) {
			...
			if (this.isAggressive() && event.isMoving()) {
				event.getController().setAnimation(new AnimationBuilder().addAnimation("aggression", EDefaultLoopTypes.LOOP));
				return PlayState.CONTINUE;
			}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 100) {
			this.remove(MutantEndermanEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
